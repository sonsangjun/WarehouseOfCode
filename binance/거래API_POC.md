    ...
    */
    ...
    const obj = {
        getAvgPrice : function(){
            ...
            const finalParam = 'symbol=BTCUSDT';

            return orderObj.xhrGet(url,finalParam).then(function(){
                console.warn('getAvgPrice',arguments);

            }).catch(function(){
                console.error('getAvgPrice',arguments);
            });
        },

        getExchangeInfo : function(symbol){
            const url = '/api/v3/exchangeInfo';
            const finalParam = '';

            return orderObj.xhrGet(url,finalParam).then(function(result){
                if(!symbol){
                    return console.warn('getExchangeInfo',result);    
                }
                ...
                console.error('getExchangeInfo',arguments);
            });
        },
        ...
        getAccInfo : function(){
            ...
            An additional parameter, recvWindow, may be sent to specify the number of milliseconds after timestamp the request is valid for. If recvWindow is not sent, it defaults to 5000.
             란 말이 있어 파라메터로 보내고 있는데 전혀먹히지 않는 모양
             5000이외 값 설정시 fail 떨어진다.

             그외, URL상에서 api/v3/time등의 API호출후
             ...
             오류메시지가 나타나면, 그 브라우저 안에서 다른 API호출시 recvwindow를 초과했다는 오류메시지가
             나타나면서 정상동작하지 않을 가능성이 있다.
             해당 메시지가 나타나면 그냥 api.binance.com에서만 테스트 하기.
            

            const recvWindow = 5000; 
            const timestampLong = (new Date()).getTime();
            const offsetTimeStr = timestampLong - recvWindow; ==> recvWindow 안빼도됨
            */
            // 윈도우가 응답받기까지의 시간 텀
            /////////////////////////////////
            const recvWindow = 5000; 
            ...
            const rawParam = 'recvWindow='+recvWindow+'&timestamp='+offsetTimeStr;
            const signatureStr = GenerateHMAC(dealSet.APIPkey,rawParam);

            const finalParam = rawParam + '&signature=' + signatureStr;

            return orderObj.xhrGet(url,finalParam).then(function(){
                console.warn('getAccInfo',arguments);
            }).catch(function(){
                console.error('getAccInfo',arguments);
            });
        },

        callOrderTest : function(){
            obj.callOrder(true);
        },

        callOrderMarketTest : function(){
            obj.callOrderMarket(true);
        },

        // 지정가 주문
        callOrder : function(isTest){
            /////////////////////////////////
            // 윈도우가 응답받기까지의 시간 텀
            ...
            const url = (isTest ? '/api/v3/order/test' : '/api/v3/order');
            const rawParam = 
                 'symbol=BTCUSDT'+'&'
                +'side=BUY'+'&'
                +'type=LIMIT'+'&'
            ...
            return orderObj.xhrPost(url,finalParam).then(function(){
                console.warn('getAccInfo',arguments);
            }).catch(function(){
                console.error('getAccInfo',arguments);
            });
        ...
        */
        callOrderMarket : function(isTest){
            /////////////////////////////////
            // 윈도우가 응답받기까지의 시간 텀
        ...
        xhrGet : function(url,param){
            const ajaxObj = new Promise((resolve,reject)=>{
                ...
                xhr.open('GET',dealSet.baseUrl+url+'?'+param);
        ...
        xhrPost : function(url,param){
            const ajaxObj = new Promise((resolve,reject)=>{
                ...
                xhr.open('POST',dealSet.baseUrl+url);
                xhr.setRequestHeader('X-MBX-APIKEY',dealSet.APIKey);
                xhr.send(param);
            });
